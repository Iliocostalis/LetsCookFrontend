...
public class OnlineRepositoryPutTest {
    ...
    public void setUp() throws Exception {
        ...
        // Save Mock-PutRecipeRequest with linked Data and return Mock-PutRecipeRequest
        PowerMockito.whenNew(PutRecipeRequest.class).withAnyArguments().then(invocation -> {
            PutRecipeRequest mockRequest = Mockito.mock(PutRecipeRequest.class);
            testHelper.saveRequest(mockRequest, invocation);
            return mockRequest;
        });

        // Add PutRecipeRequest to Queue (like normal RequestQueue.add(Request)) -> Handled in TestHelper
        Mockito.when(mockRequestQueue.add(any(PutRecipeRequest.class))).thenAnswer(invocation -> {
    ...
    @Test
    public void correctApiCallForPutRecipe() {
        ...
        Recipe recipe = new Recipe("1","Titel", 2, "Beschreibung", ingredients, instructions, null);

        OnlineRepository.getRepository(con).putRecipe(recipe);

        // List of expected executed Requests
        ArrayList<OnlineRepositoryTestHelper.RequestData> executedRequests = new ArrayList<>();
        executedRequests.add(new OnlineRepositoryTestHelper.RequestData(Request.Method.PUT, "https://lets-cook-backend-zany-oryx-tj.cfapps.io/api/recipes/", recipe));

        testHelper.assertIfExecutedRequestsAreEqual(executedRequests);
    }

    @Test
    public void correctEnumForPutRecipe() {
        ...
        OnlineRepository.getRepository(con).putRecipe(recipe).observeForever(mockObserver);
